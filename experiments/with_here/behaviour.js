...
// Function to create the heatmap layer using heatmap.js
function createHeatmapLayer(map) {
	...
	const heatmapData = {
		max: 5,
		data: [
			{lat: 52.500, lng: 13.400, value: 3},
			{lat: 52.501, lng: 13.401, value: 4},
			{lat: 52.502, lng: 13.402, value: 5}
		]
	};

	const heatmapPoints = heatmapData.data.map(point => {
		return {
			x: map.geoToScreen({lat: point.lat, lng: point.lng}).x,
			y: map.geoToScreen({lat: point.lat, lng: point.lng}).y,
			value: point.value
		};
	});
	...
	// Update heatmap points on map view change
	map.addEventListener('mapviewchange', () => {
		const updatedPoints = heatmapData.data.map(point => {
			return {
				x: map.geoToScreen({lat: point.lat, lng: point.lng}).x,
				y: map.geoToScreen({lat: point.lat, lng: point.lng}).y,
				value: point.value
...
// Call the function to create the heatmap layer
